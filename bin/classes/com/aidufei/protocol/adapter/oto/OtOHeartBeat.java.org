...
//	private boolean mNeedReconnect = false;
	...
	private boolean connectRemote(){
		...
		try {
			Log.v(TAG,"begin connect to beat port");
...
//			mRemote.setState(Device.STATE_IDLE);		
	...
	private void createTimer(){
		if(mTimer == null){
			Log.v(TAG,"client beat timer create");
			mTimer = new Timer();
			if(mTask == null)
				mTask = new BeatTask();
	...
	private void closeTimer(){
		if(mTimer != null){
			Log.v(TAG,"client beat timer cancel");
			mTimer.cancel();			
			mTimer.purge();			
			mTimer = null;
		}
	...
	public synchronized void stop() {
		closeTimer();			
		beatDrop();	
		Log.v("ClientAdapter","client beat stop");
	}
...
//			mConRemote.setState(Device.STATE_IDLE);
		...
		Log.v(TAG,"11111111111beat drop from " + mConRemote.address());
		closeSocket();
	}
...
//			mConRemote.setState(Device.STATE_IDLE);
		...
		Log.v(TAG,"1111111111beat error from " + mConRemote.address());
		closeSocket();
	}
...
//			mConRemote.setState(Device.STATE_CONNECTED);
		...
		Log.v(TAG,"beat ok from " + mConRemote.address());
	}
