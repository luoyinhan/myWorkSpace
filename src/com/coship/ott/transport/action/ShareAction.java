...
public class ShareAction extends BaseAction {
	...
	public BaseJsonBean addFavorite(String url, String userCode,
		...
		try {
			...
			Log.d(LOGTAG, "方法addFavorite转换" + jsonData + "为BaseJsonBean时出错！");
		}
		return baseJsonBean;
	}
	...
	public FavouriteAssetListJson getFavorite(String url, String userCode,
		...
		try {
			...
			Log.d(LOGTAG, "方法getFavorite转换" + jsonData
	 ...
	 * @return {@link BaseJsonBean}
	 * */
	public BaseJsonBean deleteShare(String url, String userCode, String ids) {
		urlbuf.append("&userCode=").append(userCode).append("&ids=")
				.append(ids);
		jsonData = NetTransportUtil.getContent(url, urlbuf.toString());
		if (TextUtils.isEmpty(jsonData)) {
			return null;
		...
		try {
			...
			Log.d(LOGTAG, "方法delFavorite转换" + jsonData + "为BaseJsonBean时出错！");
		}
		return baseJsonBean;
	}
}