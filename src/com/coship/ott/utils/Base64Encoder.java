...
public class Base64Encoder {
	private final static char[] pem_array = { 65, 66, 67, 68, 69, 70, 71, 72,
	...
	public final static String encodeAtom(byte[] b, int strt, int left) {
		...
		if (left == 1) {
			...
			out.append(String
					.valueOf(pem_array[(b1 << 4 & 48) + (k >>> 4 & 15)]));
		...
		if (left == 2) {
			...
			out.append(String.valueOf(pem_array[(b2 << 4 & 48)
					+ (b4 >>> 4 & 15)]));
			out.append(String
					.valueOf(pem_array[(b4 << 2 & 60) + (l >>> 6 & 3)]));
			out.append(String.valueOf(eq));
			return out.toString();
		}
		byte b3 = b[strt];
