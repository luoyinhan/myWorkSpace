...
import com.aidufei.protocol.remote.message.KeyboardRequest;
import com.aidufei.protocol.remote.message.MSGHeadObject;
...
public class RemoteKeyboard {
	...
	public RemoteKeyboard(DeviceInfo device) {
		try {
	...
	public void remoteSendDownAndUpKeyCode(int keycode) {
		...
		Log.e("RemoteKeyboard", "send keyboard click key_value:" + key_value
				+ " shift:" + shift_press);
	}

	public void remoteSendDownOrUpKeyCode(int keycode, int event_type) {
		Log.e("remoteActivity", "keyRepeater send  key " + keycode
	...
	private void remoteSendToBoardVirtualDriver(int keycode, short event_type) {
		...
		try {
			...
			this.socket.send(p);
			Log.e("RemoteKeyboard", "send Keyboard request to driver keycode:"
					+ keycode + "eventype" + event_type);
	...
	private byte[] getKeyCodeByteData(int keycode, short event_type) {
		...
		if ((msgbuf != null) && (msg != null)) {
			Log.e("lipingning", "UDP msgLen=" + msg.length
					+ ",msgbuf.capacity()=" + msgbuf.capacity());

			msgbuf.rewind();
			msgbuf.get(msg, 0, msg.length);
			Log.e("lipingning", "after get UDP msgLen=" + msg.length);
			msgbuf.clear();
		}
		return msg;
	}
